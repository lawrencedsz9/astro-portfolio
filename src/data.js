export const profile = {
  name: 'Lawrence Robert Dsouza',
  displayName: 'Lawrence Dsouza',
  role: 'Software Engineer | Backend & AI Applications',
  location: 'Doha, Qatar',
  email: 'lowrencedsz9@gmail.com',
  website: 'lawrencedsouza.netlify.app',
  github: 'https://github.com/lawrencedsz9',
  linkedin: 'https://www.linkedin.com/in/lawrence-dsouza-a15886337',
  twitter: 'https://x.com/Lawrenc83357878?t=efLHHGdqgQQC3HCOthz63g&s=09',
  status: 'Open to backend, AI, automation, and computer vision work',
  summary:
    'Software engineer focused on backend systems, AI applications, and real-time software. I like turning practical ideas into reliable products across APIs, computer vision, automation, and realtime user experiences.',
  metaDescription:
    'Portfolio of Lawrence Robert Dsouza, a software engineer building backend systems, AI applications, real-time software, and computer vision projects.',
  hero: {
    heading: 'Lawrence Dsouza',
    text:
      'I design backend systems, AI workflows, and realtime experiences that move from prototype to usable product.',
    tags: ['FastAPI backends', 'Agentic AI', 'Computer Vision'],
  },
  stats: [
    {
      label: 'Public GitHub repos',
      value: '34',
      detail: 'experiments, apps, and systems',
    },
    {
      label: 'Base',
      value: 'Doha',
      detail: 'building with a global remote mindset',
    },
    {
      label: 'Primary stack',
      value: 'Python',
      detail: 'FastAPI, OpenCV, LangGraph, SQL',
    },
    {
      label: 'Focus',
      value: 'AI APIs',
      detail: 'voice, RAG, vision, automation',
    },
  ],
};

export const focusAreas = [
  {
    title: 'Backend APIs',
    description:
      'Designing clean service layers, REST endpoints, database-backed workflows, and async Python systems.',
  },
  {
    title: 'Agentic AI',
    description:
      'Building tool-using assistants with realtime voice, RAG workflows, function calling, and automation hooks.',
  },
  {
    title: 'Computer Vision',
    description:
      'Shipping camera, face detection, embedding search, and event capture pipelines with OpenCV and Dlib.',
  },
  {
    title: 'Realtime Software',
    description:
      'Connecting WebSockets, LiveKit, dashboards, and responsive interfaces into products that feel immediate.',
  },
];

export const skills = [
  {
    group: 'Languages',
    items: ['Python', 'C', 'SQL', 'Java', 'JavaScript'],
  },
  {
    group: 'Backend',
    items: ['FastAPI', 'Flask', 'REST APIs', 'WebSockets'],
  },
  {
    group: 'Databases',
    items: ['PostgreSQL', 'MongoDB', 'MySQL', 'SQLite', 'Firebase'],
  },
  {
    group: 'AI, Data & Tools',
    items: [
      'LangGraph',
      'YOLOv8',
      'OpenCV',
      'Dlib',
      'Computer Vision',
      'Agentic AI',
      'LLM Integration',
      'RAG Workflows',
      'Git',
      'Docker',
      'Raspberry Pi',
      'Postman',
      'LiveKit',
    ],
  },
];

export const projects = [
  {
    name: 'Smart Workspace Optimization System',
    category: 'Backend + Digital Twin',
    period: 'Oct 2025 - Jan 2026',
    description:
      'A workspace management platform with bookings, occupancy tracking, access management, and realtime facility analytics.',
    highlight:
      'Built with Next.js, FastAPI, MongoDB, WebSockets, HVAC optimization, and RAG-based natural language querying.',
    link: 'https://github.com/lawrencedsz9/WMS',
    stack: ['Next.js', 'FastAPI', 'MongoDB', 'WebSockets', 'RAG'],
  },
  {
    name: 'Nevira - Agentic Voice AI Assistant',
    category: 'Agentic Voice AI',
    period: 'Aug 2025 - Nov 2025',
    description:
      'A low-latency voice AI assistant using FastAPI, LiveKit, WebRTC, and Gemini Realtime API.',
    highlight:
      'Includes wake-word detection, async tool execution, and function calling for search, weather, and email automation.',
    link: 'https://github.com/lawrencedsz9/livekit-ai-agent',
    stack: ['FastAPI', 'LiveKit', 'WebRTC', 'Gemini', 'Python'],
  },
  {
    name: 'Camera Analysis',
    category: 'Computer Vision',
    period: 'Personal Project',
    description:
      'A realtime surveillance system with motion detection, face detection, automatic capture, and video recording.',
    highlight:
      'Uses timestamps and SQLite-backed visitor data to keep captured events organized and searchable.',
    link: 'https://github.com/lawrencedsz9/camAnalysis',
    stack: ['Python', 'OpenCV', 'SQLite'],
  },
  {
    name: 'Facial Recognition Image Retrieval',
    category: 'Computer Vision Search',
    period: 'Backend + AI',
    description:
      'A face matching and image retrieval pipeline using Dlib embeddings and cosine similarity search.',
    highlight:
      'Structured around detection, embedding generation, indexing, caching, and retrieval for maintainable search workflows.',
    link: 'https://github.com/lawrencedsz9',
    stack: ['Python', 'Dlib', 'OpenCV', 'Cosine Similarity'],
  },
  {
    name: 'Members App for a Club',
    category: 'Mobile App',
    period: 'Personal Project',
    description:
      'A club management app for members, events, login, and realtime data storage.',
    highlight:
      'Built Firebase-backed workflows for event tracking and member management.',
    link: 'https://github.com/lawrencedsz9/members-app',
    stack: ['Flutter', 'Firebase'],
  },
  {
    name: 'Student Budget Tracker',
    category: 'Finance Tool',
    period: 'Personal Project',
    description:
      'A student-focused budget tracker for managing expenses and improving day-to-day money visibility.',
    highlight:
      'Built as a lightweight finance workflow with React, Firebase, and a responsive Tailwind UI.',
    link: 'https://github.com/lawrencedsz9/stud-budget',
    stack: ['React', 'Firebase', 'Tailwind CSS'],
  },
];
